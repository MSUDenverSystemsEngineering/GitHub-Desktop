...
Try {
	...
	[string]$appVendor = 'GitHub, Inc.'
	[string]$appName = 'GitHub Desktop'
	[string]$appVersion = ''
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'GitHubDesktop' -CheckDiskSpace -PersistPrompt

		## Show Progress Message (with the default message)
		Show-InstallationProgress

		## <Perform Pre-Installation tasks here>
		$exitCode = Execute-MSI -Action Uninstall -Path "{48298E90-5D48-40E5-9838-25C718088615}" -Parameters '/qn' -PassThru
		...
		Write-Log -Message "Attempting to run GitHub Desktop installer..." -Source 'Installation' -LogType 'CMTrace'
		$exitCode = Execute-MSI -Action Install -Path "$dirFiles\GitHubDesktopSetup-x64.msi" -Parameters '/qn' -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		## The MSI puts 'GitHubDesktopDeploymentTool.exe' into '%ProgramFiles(x86)%\GitHub Desktop Deployment' and registers it as a startup app under HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Run
		## The desktop deployment tool runs per user and installs GitHub Desktop on the user profile.
		## Removing the MSI application does not remove GitHub Desktop from the individual profiles.
		## It simply prevents it from being installed on any new user profiles going forward.
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'GitHubDesktop' -CloseAppsCountdown 60
		...
		Write-Log -Message "Attempting to run GitHub Desktop Deployment Tool uninstaller..." -Source 'Uninstallation' -LogType 'CMTrace'
		Write-Log -Message "Please note, this does not remove installations from individual user profiles. It only prevents new users from getting the install." -Source 'Uninstallation' -LogType 'CMTrace'
		$exitCode = Execute-MSI -Action Uninstall -Path "{48298E90-5D48-40E5-9838-25C718088615}" -Parameters '/qn' -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU4pie4JEdQ7Zn/AD549iPNFOD
# AoCgghHXMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# MRYEFHu16RNqlAiHugh4AJHdoaU2hdVkMA0GCSqGSIb3DQEBAQUABIIBgJH/xi7D
# 0y1YVRhup65HwG5561+7bwAgwqBoxrQRYjVy/MBwWklhjIv5lJ2QYr69f/I6ciFv
# 2Y7BvNIeIKNdFCbpVzUT0JDY+BJ+7ZPTH/dSEJH+p7WRwXTA2lV8wHFZ0wAqlUnz
# eBFhhWLCY1TpCqzUqubBZ1VsijhYPBbBwMabVp59GRBsteYnc/OetL87+W6A4ooq
# YQ2xXOO9hlYORYFvBgDa+O0BjWj3tnN/XYTOVO3LLKUqifdOtVqm/dq4EaJhHUhJ
# QuMwGtgfovedmmutRzDC0Ctpp0rcpImTsLx1/5jJRm3s6/0XdLdjZrGjESAyzr27
# mUcBoVx63j4TQaCJP/5uP56uXyoPm2nwzhXDKBOtahoGRZey1kFgosVub3+ydU1w
# cgObLojmGrNKHFBxetJM5hURUSz7rzHdCmIQfzxUXhi5VfjydQb2xZOUIqmJKEcc
# CbAqPvLhASCBi5KcLaVEv6ity2qyMrWBp2sfkAIx08Qtyj4gTG7WdxFajg==
